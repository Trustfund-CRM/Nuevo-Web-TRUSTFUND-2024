...
export default function SectionBottom() {
    ...
    async function fetchData() {
      try {
        const response = await axios.get(
          `https://trustfundcrm.testeoenzo.online/api/inmobiliarias_adheridas`
        );
  ...
  function filtrarProvincia(e) {
    ...
    let temp =
      estadito &&
      estadito.filter((estado) => estado.provincia === e.target.value);
  ...
  function handleFilter() {
    setInmobiliarias(
      estadito.filter(
        (inmo) => inmo.provincia === provincia && inmo.localidad === localidad
      )
    );
  }
                        ...
                        <div key={inmo.id} className={style.Card}>
                          <Image
                            ...
                            src={
                              inmo.logo
                                ? inmo.logo ===
                                  "https://res.cloudinary.com/trustfund2022/image/upload/f_auto,q_auto/n0stapp60q7el4rtdcqs"
                                  ? logo404
                                  : inmo.logo || ""
                                : logo404
                            }
